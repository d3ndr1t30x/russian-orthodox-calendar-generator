...
class NewProjectDialog(QDialog):
    def __init__(self, defaults=None, parent=None):
        ...
        self.week_tone = QCheckBox(); self.week_tone.setChecked(getattr(defaults, "include_liturgical_week_tone", True))
        ...
        form = QFormLayout(); form.addRow("Project name", self.name); form.addRow("Gregorian calendar year", self.year); form.addRow("Australian state / territory", self.state); form.addRow("Calendar language", self.language); form.addRow("Template", self.template); form.addRow("Paper", self.paper); form.addRow("Orientation", self.orientation); form.addRow("Show liturgical week / tone", self.week_tone)
    ...
    def project_settings(self, defaults=None) -> ProjectSettings:
        return ProjectSettings(
            ...
            include_service_rank_legend=getattr(defaults, "include_service_rank_legend", True), include_liturgical_week_tone=self.week_tone.isChecked(), rank_labels_en=dict(getattr(defaults, "rank_labels_en", {})),
